...
vec3	computeAmbientBRDF(vec3 surfToView, vec3 surfaceNormal, float roughness, float metalness, vec3 surfaceColor FS_ARGS)
{
	...
	const float	kCubemapMipRange = 7;
	...
	// Ambient diffuse
	vec3	irradiance = SAMPLELOD_CUBE(EnvironmentMapSampler, surfaceNormal, kCubemapMipRange).rgb;
	vec3	diffuse = irradiance * surfaceColor;

	// Ambient specular
	vec3	prefilteredColor = SAMPLELOD_CUBE(EnvironmentMapSampler, reflect(-surfToView, surfaceNormal), roughness * kCubemapMipRange).rgb;
...
#   if      defined(HAS_Atlas)
	...
	if (blendingType == 2)
	{
		...
		float	cursor = fract(fInput.fragAtlasID);
	...
	{
		// Linear
		blendMix = fract(fInput.fragAtlasID);
	}
...
#	if	defined(HAS_Atlas)
	rect0 = fInput.fragRect0;
	vec4	rect1 = fInput.fragRect1;
...
#	endif
	...
	color.rgb =  SAMPLE(DiffuseRamp_RampMap, vec2(color.x,0.0)).rgb;
#endif
...
#	endif
	...
	vec3 surfaceNormal = mul(TBN, normalTex1.xyz);
	...
	vec3    surfToLight = -GET_CONSTANT(SceneInfo, DirLightPosition).xyz;
	vec3    surfToLightDir = normalize(surfToLight);
	vec3    surfToViewDir = normalize(surfToView);

	vec3	lightColor = GET_CONSTANT(SceneInfo, DirLightColor).rgb * GET_CONSTANT(SceneInfo, DirLightColor).a;
	...
	vec3	ambientColor = GET_CONSTANT(SceneInfo, AmbientLightColor).rgb * GET_CONSTANT(SceneInfo, AmbientLightColor).a;
...
#	endif
	...
	vec3    posToLightDirection = - normalize(GET_CONSTANT(SceneInfo, DirLightPosition).xyz); // for the directional lights, instead of the position, we get the orientation
	...
	float   lightIntensity = GET_CONSTANT(SceneInfo, DirLightColor).a;
	//  Compute the ambient light
	vec3    lightAmbient =  GET_CONSTANT(SceneInfo, AmbientLightColor).rgb * GET_CONSTANT(SceneInfo, AmbientLightColor).a +
							GET_CONSTANT(Material, LegacyLit_AmbientLight);
	// Final light factor
	color.rgb *= (lightIntensity * GET_CONSTANT(SceneInfo, DirLightColor).rgb * lightIncidence + lightAmbient) * GET_CONSTANT(Material, LegacyLit_LightScale);
#endif
